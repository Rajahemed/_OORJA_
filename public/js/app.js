            ...
            if (result.success) {
                ...
                if (breakdownContainer && breakdownList && s.referralBreakdown) {
                    ...
                    for (let i = 9; i >= 3; i--) {
                        ...
                        let levelText = window.i18next ? window.i18next.t('level_points', { level, points: i }) : `Level ${level} (${i} Points)`;
                        if(levelText === 'level_points') levelText = `Level ${level} (${i} Points)`;
                        
                        let referralText = window.i18next ? window.i18next.t(count === 1 ? 'referrals_count_one' : 'referrals_count_other', { count }) : `${count} Referral${count !== 1 ? 's' : ''}`;
                        if(referralText === 'referrals_count_one' || referralText === 'referrals_count_other') referralText = `${count} Referral${count !== 1 ? 's' : ''}`;
