...
async function openDataDrilldown(type) {
    ...
    function changeLanguage(lang) {
        ...
        window.lastGeneratedWhatsAppPosterUrl = null;
        ...
        if (waImgEl && imgMap[lang]) {
            const userCode = (typeof currentUser !== 'undefined' && currentUser && currentUser.referralCode) || 
                             window.lastRegisteredReferralCode || 'RWPRO';
            if (typeof window.generateReferralPoster === 'function') {
                window.generateReferralPoster(userCode, lang, function(dataUrl) {
                    if (dataUrl && waImgEl) {
                        waImgEl.src = dataUrl;
                        window.lastGeneratedWhatsAppPosterUrl = dataUrl;
                    }
                });
            } else {
    ...
    window.generateReferralPoster = function(referralCode, lang, callback) {
        const langMap = {
            'en': 'English',
            'hi': 'Hindi',
            'kn': 'Kannada',
        ...
        const currentLng = lang || (window.i18next && window.i18next.language) || localStorage.getItem('selectedLang') || localStorage.getItem('i18nextLng') || 'en';
        const baseLng = (currentLng || 'en').split('-')[0];
        const langName = langMap[baseLng] || 'English';
        const bannerFile = 'og-image-' + langName + '.webp';
        ...
        img.onload = function() {
            ...
            const code = referralCode || (typeof currentUser !== 'undefined' && currentUser && currentUser.referralCode) || window.lastRegisteredReferralCode || 'RWPRO';
            ...
            const renderPoster = function(sourceQr) {
                if (sourceQr) {
                    // Position matching the Welcome page poster:
                    // position: absolute; bottom: 14%; right: 4%; width: 23%; aspect-ratio: 1/1; padding: 5px;
                    const qrWidth = canvas.width * 0.23;
                    const qrHeight = qrWidth;
                    const x = canvas.width * (1 - 0.04) - qrWidth;
                    const y = canvas.height * (1 - 0.14) - qrHeight;
                    const padding = Math.round(canvas.width * 0.005); // ~5px on 1024px

                    ctx.fillStyle = '#ffffff';
                    if (ctx.roundRect) {
                        ctx.beginPath();
                        ctx.roundRect(x - padding, y - padding, qrWidth + (padding * 2), qrHeight + (padding * 2), 6);
                        ctx.fill();
                    } else {
                        ctx.fillRect(x - padding, y - padding, qrWidth + (padding * 2), qrHeight + (padding * 2));
                    }
                    ctx.drawImage(sourceQr, x, y, qrWidth, qrHeight);
                }

                // Preserve branding completely. Do not draw over Oorja logo.
                const dataUrl = canvas.toDataURL('image/png', 1.0);
                window.lastGeneratedWhatsAppPosterUrl = dataUrl;
                window.lastRegisteredReferralCode = code;
                window.lastGeneratedPosterLang = baseLng;
            ...
            if (qrCanvas) {
                ...
                qrImg.src = 'https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=' + encodeURIComponent(qrUrl) + '&color=000000&bgcolor=ffffff';
            }
        };
        img.onerror = function(err) {
            console.error('Failed to load poster image for referral generation:', bannerSrc, err);
            if (typeof callback === 'function') {
                callback(null, null);
            }
        };
    ...
    window.showReferralQR = function() {
        ...
        const currentLng = (window.i18next && window.i18next.language) || localStorage.getItem('selectedLang') || 'en';
        const baseLng = currentLng.split('-')[0];
        const imgMap = { 'en': 'English', 'hi': 'Hindi', 'kn': 'Kannada', 'ta': 'Tamil', 'te': 'Telugu', 'mr': 'Marathi', 'gu': 'Gujarati', 'bn': 'Bengali' };
        const qrModalImgEl = document.getElementById('qrModalPosterImg');
        if (qrModalImgEl && imgMap[baseLng]) {
            qrModalImgEl.src = '/og-image-' + imgMap[baseLng] + '.webp';
        }
    ...
    window.shareReferralWhatsApp = async function() {
        ...
        if (typeof window.shareWithImage === 'function') {
            await window.shareWithImage(null, currentUser.referralCode, currentUser.fullName);
        }
    };
...
function submitRegistration() {
        ...
        const topRidersSection = document.getElementById('topRidersSection');
            ...
            if (topRidersSection) topRidersSection.style.display = 'none';
                ...
                // Dynamically render the banner with exact QR using single source of truth
                window.lastRegisteredReferralCode = result.referralCode;
                if (typeof window.generateReferralPoster === 'function') {
                    window.generateReferralPoster(result.referralCode, baseLng, function(finalDataUrl) {
                        if (finalDataUrl) {
                            const waBanner = document.getElementById('whatsappBannerImage');
                            if (waBanner) waBanner.src = finalDataUrl;
    ...
    window.shareWithImage = async function(e, specificCode = null, specificName = null) {
        ...
        const code = specificCode || (typeof currentUser !== 'undefined' && currentUser ? currentUser.referralCode : (window.lastRegisteredReferralCode || 'RWPRO'));
        const fullName = specificName || (typeof currentUser !== 'undefined' && currentUser ? currentUser.fullName : 'Rider');
        ...
        const currentLng = (window.i18next && window.i18next.language) || localStorage.getItem('selectedLang') || localStorage.getItem('i18nextLng') || 'en';
        const baseLng = (currentLng || 'en').split('-')[0];
        ...
        if (window.lastGeneratedWhatsAppPosterUrl && window.lastRegisteredReferralCode === code && window.lastGeneratedPosterLang === baseLng) {
            await doShare(window.lastGeneratedWhatsAppPosterUrl);
        } else if (typeof window.generateReferralPoster === 'function') {
            window.generateReferralPoster(code, baseLng, async function(posterDataUrl) {
                await doShare(posterDataUrl);
            });
        } else {
            await doShare(null);
        }
    ...
    document.addEventListener('mouseleave', (e) => {
        ...
        if (e.clientY < 0 && !exitIntentTriggered) {
            ...
            if (modal && !modal.classList.contains('show')) {
                ...
                if (title) {
                    title.innerHTML = '🎁 Wait! Claim Your FREE 10-Point Joining Reward!';
                }
